...
版本 **0.2.0**。Entropy、Lighter、Robinhood Lighter、QFEX 四平台工具现已接入 pandazhai.com 管理中心。
...
封包不含松散 Python 应用源码、账户配置或数据库；不能保证防止专业反编译。保留独立版 `install-local.sh`，需要独立部署时可使用。新建组合默认滑动窗口策略，各组合目标独立设置；旧组合保留原模式。见 [0.2.0 版本说明](RELEASE-0.2.0.md)。
