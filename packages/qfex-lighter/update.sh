...
exec 9>/var/lock/pandazhai-qfex-update.lock
flock -n 9 || { echo '已有 QFEX 更新进程，请勿同时更新。' >&2; exit 1; }
prefix=/opt/pandazhai/qfex
data_dir=/var/lib/pandazhai/qfex
service=panda-qfex.service
if [[ -x "$prefix/panda-arb" ]]; then
  target="$prefix/panda-arb"
elif [[ -x "$prefix/panda-arb-qfex" ]]; then
  target="$prefix/panda-arb-qfex"
else
  echo '未找到已安装的 QFEX-Lighter 程序；本脚本仅用于更新。' >&2
  exit 1
fi
[[ -d "$data_dir" ]] || { echo '未找到原节点数据目录，停止更新。' >&2; exit 1; }
systemctl cat "$service" >/dev/null
temporary=$(mktemp -d /tmp/pandazhai-qfex-update.XXXXXX)
stopped=0
replaced=0
success=0
was_active=0
cleanup() {
  ...
  if (( stopped && !success )); then
    ...
    if (( was_active && !replaced )); then
      systemctl start "$service" || echo '原服务启动失败，请检查 systemctl status panda-qfex.service。' >&2
    fi
  fi
  rm -rf -- "$temporary"
  exit "$result"
...
release_ref="main"
base="https://raw.githubusercontent.com/lihanyu81/PandaZhai-Arbitrage-Tool/$release_ref/packages/qfex-lighter"
...
backup="/var/backups/pandazhai/qfex/$(date -u +%Y%m%dT%H%M%SZ)-$$"
...
check_state
tar -C /var/lib/pandazhai -czf "$backup/qfex-data.tar.gz" qfex
chmod 600 "$backup/qfex-data.tar.gz"
tar -tzf "$backup/qfex-data.tar.gz" >/dev/null
...
success=1
echo "已更新至 QFEX-Lighter 0.5.0，备份目录：$backup"
