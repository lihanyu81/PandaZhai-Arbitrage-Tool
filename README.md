...
本仓库提供熊猫寨多服务器管理系统使用的 Linux x86_64 节点封包。套利工具以独立可执行文件发布；仓库另外开放了独立的 POPDEX Agent 钱包辅助程序源码。仓库不包含用户配置、数据库、私钥、API Key、助记词或 `.env`。
...
已移除 PopDEX-Lighter、RBLighter-Lighter、Entropy-Lighter 发布包中误收集的明文业务 Python 文件。此次仅重新封装资源，原有运行代码和依赖内容不变，工具版本号不变；封包修订号为 `20260912-source-clean`，可查看各目录的 `PACKAGING.json`。安装和更新入口使用修复后的封包及对应 SHA-256。

当前六组节点的 12 个可执行文件已检查，不含松散的业务 `.py` / `.pyc` 文件。PyInstaller 的 PYZ 字节码仍可被分析，此修复不等于源码加密或防反编译。Git 历史中的旧封包及已经下载的副本不受本次修复影响。独立的 `agent-wallet-source` 辅助程序继续按原计划公开。
