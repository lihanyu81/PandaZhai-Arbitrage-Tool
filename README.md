...
RBLighter 和 Entropy 新版均不再要求 `.env`。工具界面中的“配置/运行配置”可以填写对应交易所和 Telegram 配置，内容分别保存在节点数据目录下的 `tool-data/config.json`。保存后请在管理中心重启对应节点；API 私钥和 Bot Token 不会回显到浏览器。Entropy 节点的配置路径为 `entropy-node/tool-data/config.json`。
